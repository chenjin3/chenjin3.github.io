...
在JavaScript中，闭包结构可以用于创建一个有用的设计模式，被称为部分应用（在纯粹的函数式语言中，一般不说函数是被调用的，而说函数是被应用的）或函数工厂（function factories）。部分应用允许我们手动绑定一个或多个预设的实参到一个通用的函数，该函数返回一个部分应用的更具体的函数，它可以接收剩下的其他参数。
